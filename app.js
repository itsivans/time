...
function calcolaPercentualiSettimana(filtro = "all") {
  ...
  db.collection("activities")
    ...
    .get().then(snapshot => {
      ...
      // Estende l’ultima attività del range fino a fine periodo
      let nextDay = new Date(end);
      nextDay.setDate(nextDay.getDate() + 1);
      let weekEnd = nextDay.toISOString().slice(0,10) + "T00:00";
      // Cerca la prima attività DOPO il range per riempire eventuale buco
      db.collection("activities")
        ...
        .get().then(nextSnap => {
          ...
          let risultato = `<b>Statistiche dal ${start} al ${end} (${filtro==='free'?'solo tempo gestibile':'tutte le attività'})</b>
          <br>(Totale minuti: <b>${totaliMinuti}</b>)<br><table><tr><th>Tag</th><th>Minuti</th><th>%</th></tr>`;
          Object.entries(tempoPerTag).forEach(([tag, minuti]) => {
            let perc = ((minuti / totaliMinuti) * 100).toFixed(1);
            risultato += `<tr><td>${tag}</td><td>${minuti}</td><td>${perc}%</td></tr>`;
          });
